...
- **시설물 탭 및 행정구역 연쇄 select 컨벤션**:
  ...
  - **핀 묶어 보기(클러스터링) 토글**: 내업 상태 필터 줄 아래의 한 줄 체크박스(`#facilityClusterToggle`, `.facility-cluster-toggle`, 기본 켬). **지도 표시 방식만 바꾸며 목록·건수에는 영향이 없습니다**(필터가 아니므로 세그먼트·select와 시각적으로 구분되게 둘 것). 겹치는 핀을 개수 배지로 묶고, 배지를 누르면 그 범위로 확대합니다. **같은 건물처럼 좌표가 같아 확대해도 겹쳐 있는 시설물은 배율 18 이상(건물 하나가 화면을 채울 정도)에서 저절로 흩어집니다** — 그때 묶음 배지는 사라지고 핀만 남으며(연결선 없음), 축소하면 다시 묶입니다. 배지를 누르면 그 배율까지 확대되어 같은 결과가 됩니다. 펼쳐진 핀은 평소처럼 눌러 상세를 볼 수 있습니다. 묶음에는 **현재 필터를 통과한 피처만** 들어가므로 배지 숫자와 목록 건수가 항상 일치합니다. 선택은 `localStorage`(`sjLabFacilityCluster`)에 기억합니다. 동작·구현 규칙은 `docs/map-architecture.md`의 "핀 묶음(클러스터링)" 참고.
