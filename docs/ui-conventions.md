...
- **시설물 탭 및 행정구역 연쇄 select 컨벤션**:
  ...
  - **핀 묶어 보기(클러스터링) 토글**: 내업 상태 필터 줄 아래의 한 줄 체크박스(`#facilityClusterToggle`, `.facility-cluster-toggle`, 기본 켬). **지도 표시 방식만 바꾸며 목록·건수에는 영향이 없습니다**(필터가 아니므로 세그먼트·select와 시각적으로 구분되게 둘 것). 겹치는 핀을 개수 배지로 묶고, 배지를 누르면 그 범위로 확대합니다. **배율 18 이상(건물 하나가 화면을 채울 정도)으로 확대하면 그때까지 묶여 있던 시설물이 저절로 흩어집니다**(좌표가 완전히 같은 경우뿐 아니라, 몇 미터 간격이라 계속 묶이던 무리도 포함) — 그때 묶음 배지는 사라지고 핀만 남으며(연결선 없음), 축소하면 다시 묶입니다. 배지를 누르면 그 배율까지 확대되어 같은 결과가 됩니다. 펼쳐진 핀은 평소처럼 눌러 상세를 볼 수 있습니다. 묶음에는 **현재 필터를 통과한 피처만** 들어가므로 배지 숫자와 목록 건수가 항상 일치합니다. 선택은 `localStorage`(`sjLabFacilityCluster`)에 기억합니다. 동작·구현 규칙은 `docs/map-architecture.md`의 "핀 묶음(클러스터링)" 참고.
  ...
  - **내업 보고서 PDF**: 내업 기록 카드의 액션 줄에 `보고서` 버튼이 붙는데, **상태가 완료(`DONE`)·보류(`HOLD`)인 기록에만** 만듭니다(백엔드도 그 밖의 상태는 거부하므로 조건을 한쪽만 바꾸지 말 것). 누르면 `GET /map/qfield/facilities/{totalId}/report/pdf?workId=`를 **fetch 로 받아 Blob 으로 저장**합니다 — 새 탭으로 열면 브라우저 뷰어가 뜨고 파일명이 사라집니다. 파일명은 응답 `Content-Disposition`의 `filename*=UTF-8''`를 우선 쓰고(`parseContentDispositionFileName()`), 없으면 `내업보고서_<totalId>_<날짜>.pdf`로 대체합니다. 사진이 많으면 몇 초 걸리므로 버튼을 잠그고 "만드는 중..."을 표시하며, 오류는 `showFacilityOfficeNotice()`로 영역 안에 보여줍니다(404=기록 없음, 400·409=완료·보류만 가능).
